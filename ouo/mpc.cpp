...
static mpc_input_t *mpc_input_new_string(const char *filename, const char *string) {
  ...
  i->marks = (mpc_state_t *)malloc(sizeof(mpc_state_t) * i->marks_slots);
  i->lasts = (char *)malloc(sizeof(char) * i->marks_slots);
...
typedef union {
  ...
  mpc_pdata_not_t not_t;
  mpc_pdata_repeat_t repeat;
  mpc_pdata_and_t and_t;
  mpc_pdata_or_t or_t;
} mpc_pdata_t;
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_APPLY:
      if (mpc_parse_input(i, p->data.apply.x, r)) {
        ...
        MPC_FAILURE((mpc_err_t *)r->output);
      }
    ...
    case MPC_TYPE_NOT:
      mpc_input_mark(i);
      if (mpc_parse_input(i, p->data.not_t.x, r)) {
        mpc_input_rewind(i);
        p->data.not_t.dx(r->output);
        MPC_FAILURE(mpc_err_new(i->filename, i->state, "opposite", mpc_input_peekc(i)));
      } else {
        mpc_input_unmark(i);
        mpc_err_delete(r->error);
        MPC_SUCCESS(p->data.not_t.lf());
      }
    ...
    case MPC_TYPE_MANY:
      ...
      results = (mpc_result_t *)malloc(sizeof(mpc_result_t) * results_slots);
      
      while (mpc_parse_input(i, p->data.repeat.x, &results[j])) {
        j++;
        if (j >= results_slots) {
          results_slots = j + j / 2;
          results = (mpc_result_t *)realloc(results, sizeof(mpc_result_t) * results_slots);
        }
      }
    ...
    case MPC_TYPE_COUNT:
      ...
      results = p->data.repeat.n > MPC_PARSE_STACK_MIN
        ? (mpc_result_t *)malloc(sizeof(mpc_result_t) * p->data.repeat.n)
        : results_stk;
    ...
    case MPC_TYPE_OR:
      ...
      if (p->data.or_t.n == 0) { MPC_SUCCESS(NULL); }
      
      results = p->data.or_t.n > MPC_PARSE_STACK_MIN
        ? (mpc_result_t *)malloc(sizeof(mpc_result_t) * p->data.or_t.n)
        : results_stk;
      
      for (j = 0; j < p->data.or_t.n; j++) {
        if (mpc_parse_input(i, p->data.or_t.xs[j], &results[j])) {
          for (k = 0; k < j; k++) {
    ...
    case MPC_TYPE_AND:
      ...
      if (p->data.and_t.n == 0) { MPC_SUCCESS(NULL); }
      ...
      mpc_input_mark(i);
      for (j = 0; j < p->data.and_t.n; j++) {
        if (!mpc_parse_input(i, p->data.and_t.xs[j], &results[j])) {
          mpc_input_rewind(i);
          for (k = 0; k < j; k++) {
            p->data.and_t.dxs[k](results[k].output);
          }
      ...
      MPC_SUCCESS(p->data.and_t.f(j, (mpc_val_t**)results);
        if (p->data.or_t.n > MPC_PARSE_STACK_MIN) { free(results); });
...
static void mpc_undefine_or(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data.or_t.n; i++) {
    mpc_undefine_unretained(p->data.or_t.xs[i], 0);
  }
  free(p->data.or_t.xs);
...
static void mpc_undefine_and(mpc_parser_t *p) {
  ...
  int i;
  for (i = 0; i < p->data.and_t.n; i++) {
    mpc_undefine_unretained(p->data.and_t.xs[i], 0);
  }
  free(p->data.and_t.xs);
  free(p->data.and_t.dxs);
...
static void mpc_undefine_unretained(mpc_parser_t *p, int force) {
  ...
  switch (p->type) {
    ...
    case MPC_TYPE_MAYBE:
    case MPC_TYPE_NOT:
      mpc_undefine_unretained(p->data.not_t.x, 0);
      break;
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data.or_t.n = n;
  p->data.or_t.xs = (mpc_parser_t **)malloc(sizeof(mpc_parser_t*) * n);
  
  va_start(va, n);  
  for (i = 0; i < n; i++) {
    p->data.or_t.xs[i] = va_arg(va, mpc_parser_t*);
  }
  va_end(va);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data.and_t.n = n;
  p->data.and_t.f = f;
  p->data.and_t.xs = (mpc_parser_t **)malloc(sizeof(mpc_parser_t*) * n);
  p->data.and_t.dxs = (mpc_dtor_t *)malloc(sizeof(mpc_dtor_t) * (n-1));

  va_start(va, f);  
  for (i = 0; i < n; i++) {
    p->data.and_t.xs[i] = va_arg(va, mpc_parser_t*);
  }
  for (i = 0; i < (n-1); i++) {
    p->data.and_t.dxs[i] = va_arg(va, mpc_dtor_t);
  }  
  va_end(va);
...
static mpc_val_t *mpcf_re_repeat(int n, mpc_val_t **xs) {
  ...
  (void) n;
  if (xs[1] == NULL) { return xs[0]; }
  if (strcmp((const char *)xs[1], "*") == 0) { free(xs[1]); return mpc_many(mpcf_strfold, (mpc_parser_t *)xs[0]); }
  if (strcmp((const char *)xs[1], "+") == 0) { free(xs[1]); return mpc_many1(mpcf_strfold, (mpc_parser_t *)xs[0]); }
  if (strcmp((const char *)xs[1], "?") == 0) { free(xs[1]); return mpc_maybe_lift((mpc_parser_t *)xs[0], mpcf_ctor_str); }
  num = *(int*)xs[1];
  free(xs[1]);
  ...
  for (i = comp; i < strlen(s); i++){
    ...
    /* Regex Range...Range */
      ...
      if (s[i+1] == '\0' || i == 0) {
        ...
        for (j = start; j <= end; j++) {
          range = (char *)realloc(range, strlen(range) + 1 + 1 + 1);
          range[strlen(range) + 1] = '\0';
...
mpc_parser_t *mpc_re(const char *re) {
  ...
  mpc_optimise((mpc_parser_t *)r.output);
...
mpc_val_t *mpcf_int(mpc_val_t *x) {
  ...
  *y = (int)strtol((const char *)x, NULL, 10);
  free(x);
  return y;
}
...
mpc_val_t *mpcf_hex(mpc_val_t *x) {
  int *y = (int *)malloc(sizeof(int));
  *y = (int)strtol((const char *)x, NULL, 16);
  free(x);
  return y;
}
...
mpc_val_t *mpcf_oct(mpc_val_t *x) {
  int *y = (int *)malloc(sizeof(int));
  *y = (int)strtol((const char *)x, NULL, 8);
  free(x);
  return y;
}
...
mpc_val_t *mpcf_float(mpc_val_t *x) {
  ...
  *y = (float)strtod((const char *)x, NULL);
  free(x);
  return y;
}
...
mpc_val_t *mpcf_strfold(int n, mpc_val_t **xs) {
  ...
  for (i = 0; i < n; i++) { l += strlen((const char *)xs[i]); }
  
  xs[0] = realloc(xs[0], l + 1);
  
  for (i = 1; i < n; i++) {
    strcat((char *)xs[0], (const char *)xs[i]); free(xs[i]);
  }
...
mpc_val_t *mpcf_maths(int n, mpc_val_t **xs) {
  ...
  if (strcmp((const char *)xs[1], "*") == 0) { *vs[0] *= *vs[2]; }
  if (strcmp((const char *)xs[1], "/") == 0) { *vs[0] /= *vs[2]; }
  if (strcmp((const char *)xs[1], "%") == 0) { *vs[0] %= *vs[2]; }
  if (strcmp((const char *)xs[1], "+") == 0) { *vs[0] += *vs[2]; }
  if (strcmp((const char *)xs[1], "-") == 0) { *vs[0] -= *vs[2]; }
...
static void mpc_print_unretained(mpc_parser_t *p, int force) {
  ...
  if (p->type == MPC_TYPE_NOT)   { mpc_print_unretained(p->data.not_t.x, 0); printf("!"); }
  if (p->type == MPC_TYPE_MAYBE) { mpc_print_unretained(p->data.not_t.x, 0); printf("?"); }
  ...
  if (p->type == MPC_TYPE_OR) {
    printf("(");
    for(i = 0; i < p->data.or_t.n-1; i++) {
      mpc_print_unretained(p->data.or_t.xs[i], 0);
      printf(" | ");
    }
    mpc_print_unretained(p->data.or_t.xs[p->data.or_t.n-1], 0);
    printf(")");
  }
  
  if (p->type == MPC_TYPE_AND) {
    printf("(");
    for(i = 0; i < p->data.and_t.n-1; i++) {
      mpc_print_unretained(p->data.and_t.xs[i], 0);
      printf(" ");
    }
    mpc_print_unretained(p->data.and_t.xs[p->data.and_t.n-1], 0);
    printf(")");
  }
...
mpc_val_t *mpcf_str_ast(mpc_val_t *c) {
  mpc_ast_t *a = mpc_ast_new("", (const char *)c);
  free(c);
  return a;
}
...
mpc_parser_t *mpca_and(int n, ...) {
  ...
  for (i = 0; i < (n-1); i++) {
    p->data.and_t.dxs[i] = (mpc_dtor_t)mpc_ast_delete;
  }    
  va_end(va);
...
static mpc_val_t *mpcaf_grammar_repeat(int n, mpc_val_t **xs) { 
  ...
  (void) n;
  if (xs[1] == NULL) { return xs[0]; }  
  if (strcmp((const char *)xs[1], "*") == 0) { free(xs[1]); return mpca_many((mpc_parser_t *)xs[0]); }
  if (strcmp((const char *)xs[1], "+") == 0) { free(xs[1]); return mpca_many1((mpc_parser_t *)xs[0]); }
  if (strcmp((const char *)xs[1], "?") == 0) { free(xs[1]); return mpca_maybe((mpc_parser_t *)xs[0]); }
  if (strcmp((const char *)xs[1], "!") == 0) { free(xs[1]); return mpca_not((mpc_parser_t *)xs[0]); }
  num = *((int*)xs[1]);
  free(xs[1]);
...
static int mpc_nodecount_unretained(mpc_parser_t* p, int force) {
  ...
  if (p->type == MPC_TYPE_NOT)   { return 1 + mpc_nodecount_unretained(p->data.not_t.x, 0); }
  if (p->type == MPC_TYPE_MAYBE) { return 1 + mpc_nodecount_unretained(p->data.not_t.x, 0); }
  ...
  if (p->type == MPC_TYPE_OR) { 
    total = 0;
    for(i = 0; i < p->data.or_t.n; i++) {
      total += mpc_nodecount_unretained(p->data.or_t.xs[i], 0);
    }
    return total;
  }
  ...
  if (p->type == MPC_TYPE_AND) {
    total = 0;
    for(i = 0; i < p->data.and_t.n; i++) {
      total += mpc_nodecount_unretained(p->data.and_t.xs[i], 0);
    }
    return total;
  }
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  if (p->type == MPC_TYPE_NOT)      { mpc_optimise_unretained(p->data.not_t.x, 0); }
  if (p->type == MPC_TYPE_MAYBE)    { mpc_optimise_unretained(p->data.not_t.x, 0); }
  ...
  if (p->type == MPC_TYPE_OR) { 
    for(i = 0; i < p->data.or_t.n; i++) {
      mpc_optimise_unretained(p->data.or_t.xs[i], 0);
    }
  }
  
  if (p->type == MPC_TYPE_AND) {
    for(i = 0; i < p->data.and_t.n; i++) {
      mpc_optimise_unretained(p->data.and_t.xs[i], 0);
    }
  }  
  ...
  while (1) {
    ...
    /* Merge rhs `or` */
    if (p->type == MPC_TYPE_OR
    &&  p->data.or_t.xs[p->data.or_t.n-1]->type == MPC_TYPE_OR
    && !p->data.or_t.xs[p->data.or_t.n-1]->retained) {
      t = p->data.or_t.xs[p->data.or_t.n-1];
      n = p->data.or_t.n; m = t->data.or_t.n;
      p->data.or_t.n = n + m - 1;
      p->data.or_t.xs = (mpc_parser_t **)realloc(p->data.or_t.xs, sizeof(mpc_parser_t*) * (n + m -1));
      memmove(p->data.or_t.xs + n - 1, t->data.or_t.xs, m * sizeof(mpc_parser_t*));
      free(t->data.or_t.xs); free(t->name); free(t);
      continue;
    }

    /* Merge lhs `or` */
    if (p->type == MPC_TYPE_OR
    &&  p->data.or_t.xs[0]->type == MPC_TYPE_OR
    && !p->data.or_t.xs[0]->retained) {
      t = p->data.or_t.xs[0];
      n = p->data.or_t.n; m = t->data.or_t.n;
      p->data.or_t.n = n + m - 1;
      p->data.or_t.xs = (mpc_parser_t **)realloc(p->data.or_t.xs, sizeof(mpc_parser_t*) * (n + m -1));
      memmove(p->data.or_t.xs + m, t->data.or_t.xs + 1, n * sizeof(mpc_parser_t*));
      memmove(p->data.or_t.xs, t->data.or_t.xs, m * sizeof(mpc_parser_t*));
      free(t->data.or_t.xs); free(t->name); free(t);
      continue;
    }
    ...
    if (p->type == MPC_TYPE_AND
    &&  p->data.and_t.n == 2
    &&  p->data.and_t.xs[0]->type == MPC_TYPE_PASS
    && !p->data.and_t.xs[0]->retained
    &&  p->data.and_t.f == mpcf_fold_ast) {
      t = p->data.and_t.xs[1];
      mpc_delete(p->data.and_t.xs[0]);
      free(p->data.and_t.xs); free(p->data.and_t.dxs); free(p->name);
    ...
    if (p->type == MPC_TYPE_AND
    &&  p->data.and_t.f == mpcf_fold_ast
    &&  p->data.and_t.xs[0]->type == MPC_TYPE_AND
    && !p->data.and_t.xs[0]->retained
    &&  p->data.and_t.xs[0]->data.and_t.f == mpcf_fold_ast) {
      t = p->data.and_t.xs[0];
      n = p->data.and_t.n; m = t->data.and_t.n;
      p->data.and_t.n = n + m - 1;
      p->data.and_t.xs = (mpc_parser_t **)realloc(p->data.and_t.xs, sizeof(mpc_parser_t*) * (n + m - 1));
      p->data.and_t.dxs = (mpc_dtor_t *)realloc(p->data.and_t.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
      memmove(p->data.and_t.xs + m, p->data.and_t.xs + 1, (n - 1) * sizeof(mpc_parser_t*));
      memmove(p->data.and_t.xs, t->data.and_t.xs, m * sizeof(mpc_parser_t*));
      for (i = 0; i < p->data.and_t.n-1; i++) { p->data.and_t.dxs[i] = (mpc_dtor_t)mpc_ast_delete; }
      free(t->data.and_t.xs); free(t->data.and_t.dxs); free(t->name); free(t); 
      continue;
    }
    ...
    if (p->type == MPC_TYPE_AND
    &&  p->data.and_t.f == mpcf_fold_ast
    &&  p->data.and_t.xs[p->data.and_t.n-1]->type == MPC_TYPE_AND
    && !p->data.and_t.xs[p->data.and_t.n-1]->retained
    &&  p->data.and_t.xs[p->data.and_t.n-1]->data.and_t.f == mpcf_fold_ast) {
      t = p->data.and_t.xs[p->data.and_t.n-1];
      n = p->data.and_t.n; m = t->data.and_t.n;
      p->data.and_t.n = n + m - 1;
      p->data.and_t.xs = (mpc_parser_t **)realloc(p->data.and_t.xs, sizeof(mpc_parser_t*) * (n + m -1));
      p->data.and_t.dxs = (mpc_dtor_t *)realloc(p->data.and_t.dxs, sizeof(mpc_dtor_t) * (n + m - 1 - 1));
      memmove(p->data.and_t.xs + n - 1, t->data.and_t.xs, m * sizeof(mpc_parser_t*));
      for (i = 0; i < p->data.and_t.n-1; i++) { p->data.and_t.dxs[i] = (mpc_dtor_t)mpc_ast_delete; }
      free(t->data.and_t.xs); free(t->data.and_t.dxs); free(t->name); free(t); 
      continue;
    }
    ...
    if (p->type == MPC_TYPE_AND
    &&  p->data.and_t.n == 2
    &&  p->data.and_t.xs[0]->type == MPC_TYPE_LIFT
    &&  p->data.and_t.xs[0]->data.lift.lf == mpcf_ctor_str
    && !p->data.and_t.xs[0]->retained
    &&  p->data.and_t.f == mpcf_strfold) {
      t = p->data.and_t.xs[1];
      mpc_delete(p->data.and_t.xs[0]);
      free(p->data.and_t.xs); free(p->data.and_t.dxs); free(p->name);
      memcpy(p, t, sizeof(mpc_parser_t));
    ...
    &&  p->data.and_t.xs[0]->type == MPC_TYPE_AND
    && !p->data.and_t.xs[0]->retained
    &&  p->data.and_t.xs[0]->data.and_t.f == mpcf_strfold) {
      t = p->data.and_t.xs[0];
      n = p->data.and_t.n; m = t->data.and_t.n;
      p->data.and_t.n = n + m - 1;
      p->data.and_t.xs = (mpc_parser_t **)realloc(p->data.and_t.xs, sizeof(mpc_parser_t*) * (n + m - 1));
      ...
      memmove(p->data.and_t.xs + m, p->data.and_t.xs + 1, (n - 1) * sizeof(mpc_parser_t*));
      memmove(p->data.and_t.xs, t->data.and_t.xs, m * sizeof(mpc_parser_t*));
      for (i = 0; i < p->data.and_t.n-1; i++) { p->data.and_t.dxs[i] = free; }
      free(t->data.and_t.xs); free(t->data.and_t.dxs); free(t->name); free(t); 
      continue;
    }
    ...
    &&  p->data.and_t.xs[p->data.and_t.n-1]->type == MPC_TYPE_AND
    && !p->data.and_t.xs[p->data.and_t.n-1]->retained
    &&  p->data.and_t.xs[p->data.and_t.n-1]->data.and_t.f == mpcf_strfold) {
      t = p->data.and_t.xs[p->data.and_t.n-1];
      n = p->data.and_t.n; m = t->data.and_t.n;
      p->data.and_t.n = n + m - 1;
      p->data.and_t.xs = (mpc_parser_t **)realloc(p->data.and_t.xs, sizeof(mpc_parser_t*) * (n + m -1));
